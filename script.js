  ...
  geocoder.geocode({'location': latlng}, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      if (results[0] && results[1]) {
        ...
        //use first element of results table to show the full address
...
const mist = (color) => {
  ...
  weatherDataIcon.then(function(apiOutput) {
    ...
    bgElements.forEach((element) => {
      ...
      } else if (['01d', '02d', '03d', '04d', '09d', '13d', '50d'].includes(icon)) {
        element.style.backgroundColor = daily;
      } else if (['01n', '02n', '03n', '04n', '09n', '13n', '50n'].includes(icon)) {
